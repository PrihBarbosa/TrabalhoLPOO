...
public class CaixaDao {
	...
	public ArrayList<Caixa> all() throws SQLException
	{
		...
		for(int i=0; i<resultado.size(); i++)
		{
			...
			Caixa caixas = new Caixa(Integer.parseInt(array[0]), Double.parseDouble(array[1]), Double.parseDouble(array[2]));
			caixa.add(caixas);
		}
		return caixa;
	}
	...
	public boolean insert(Caixa caixa)
	{
		try{
			...
			map.put("valor_abertura", caixa.getValorAbertura());
			map.put("valor_fechamento", caixa.getValorFechamento());
	...
	public boolean update(Caixa caixa)
	{
		...
		if(caixa.getValorAbertura() != 0)
			novos.put("valor_abertura", caixa.getValorAbertura());
		if(caixa.getValorAbertura() != 0)
			novos.put("valor_fechamento", caixa.getValorFechamento());
